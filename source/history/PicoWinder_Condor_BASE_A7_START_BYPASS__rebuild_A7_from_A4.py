...
ASM = ROOT / "a7_start_bypass.s"
OBJ = ROOT / "a7_start_bypass.o"
BIN = ROOT / "a7_start_bypass.bin"
RAW_OUT = ROOT / "PicoWinder_Condor_BASE_A7_START_BYPASS.raw"
UF2_OUT = ROOT / "PicoWinder_Condor_BASE_A7_START_BYPASS.uf2"

subprocess.run([
    "/usr/local/swift/usr/bin/clang",
    "--target=arm-none-eabi","-mcpu=cortex-m0plus","-mthumb",
    "-c",str(ASM),"-o",str(OBJ)],check=True)
subprocess.run([
    "/usr/local/swift/usr/bin/llvm-objcopy",
    "-O","binary","--only-section=.text.start_bypass",
    str(OBJ),str(BIN)],check=True)

wrapper=BIN.read_bytes()
assert wrapper==bytes.fromhex(
    "20 28 01 d0 01 4b 18 47 70 47 00 00 b5 62 00 10")
...
raw[0x6400:0x6400+len(wrapper)]=wrapper
