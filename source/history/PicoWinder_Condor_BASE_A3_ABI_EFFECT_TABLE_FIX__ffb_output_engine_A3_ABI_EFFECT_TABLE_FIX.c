...
#define EFFECTS_ASSIGNED ((volatile uint8_t*)0x200016d0u) /* A3: exact ABI of stock binary */
...
static void device_control(const uint8_t*b,uint16_t n) {
    ...
    if(c==1u)ctrl2(0x02u);          /* enable actuators */
        ...
        ctrl2(0x01u);
        ctrl2(0x06u); /* A2/A3: Reset re-enables native autocentre; force it OFF again. */
